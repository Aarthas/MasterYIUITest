...
//		closeWebDriver();
    ...
    private void parseCaseClasses() throws Exception {
        ...
        for (Class clz : caseClasses) {
            ...
            List<Method> filterMethods = Arrays.stream(methods).filter(new Predicate<Method>() {
                @Override
                public boolean test(Method method) {
                    ...
                    .sorted(new Comparator<Method>() {
                        ...
                        public int compare(Method method1, Method method2) {
                            UseCase useCase = method1.getAnnotation(UseCase.class);
                            UseCase useCase2 = method2.getAnnotation(UseCase.class);
                            return useCase.order() - useCase2.order();
                        }
                    }).collect(Collectors.toList());
            for (Method m : filterMethods) {

                UseCase uc = m.getAnnotation(UseCase.class);
                String point = uc.point();


                ExecuteCaseModel caseModel = new ExecuteCaseModel();
                caseModel.setName(StrUtil.isBlank(uc.name()) ? m.getName() : uc.name());
                caseModel.point = point;
