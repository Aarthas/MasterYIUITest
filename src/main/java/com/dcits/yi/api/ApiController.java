	...
	public ReturnJSONObject getReport(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StrUtil.isEmpty(fileName)) {
			...
			if (!FileUtil.exist(GlobalTestConfig.ENV_INFO.getReportFolder() + "/"
				...
				// 如果报告文件不存在则生成ztest报告
				new ZTestReportManager().create(GlobalTestConfig.report);
			}
		...
		response.setHeader("Content-Disposition", "attachment;fileName=WebTestReport.html");
		...
		try {
			...
			logger.error(e, "文件[{}]下载过程中出错...!", reportFile);
		} finally {
			if (bis != null) {
				try {
					bis.close();
